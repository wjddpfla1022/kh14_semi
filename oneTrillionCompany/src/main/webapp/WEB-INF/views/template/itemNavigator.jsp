        ...
        <div class="row">
            <div class="pagenation">
				...
				<c:if test="${itemPageVO.hasPrev()}">
					<a href="list?column=${itemPageVO.column}&keyword=${itemoPageVO.keyword}&page=${itemPageVO.getPrevBlock()}" ></a>
				</c:if>
				...
				<c:forEach var="n" begin="${itemPageVO.getStartBlock()}" end="${itemPageVO.getFinishBlock()}" step="1">
						<c:choose>
							<c:when test="${itemPageVO.page == n}">
								...
								<c:otherwise>
									<a href="list?column=${itemPageVO.column}&keyword=${itemPageVO.keyword}&page=${n}">${n}</a>
								</c:otherwise>
						</c:choose>
				</c:forEach>
				...
				<c:if test="${itemPageVO.hasNext()}">
					<a href="list?column=${itemPageVO.column}&keyword=${itemPageVO.keyword}&page=${itemPageVO.getNextBlock()}" >
