...
<div class="container w-1000 my-30">
	...
	<div class="row center">
		<c:choose>
			<c:when test="${sessionScope.createdLevel == '일반회원'}">
							...
							<tr class="tr-table-bottom">
								...
								<td class="td-table-bottom"><c:choose>
										...
										<c:otherwise>
											<c:choose>
												<c:when
													test="${fn:length(reviewDto.reviewWriterString) > 3}">
													<%-- 작성자의 아이디를 3글자 추출 후 표시 --%>
													<c:out
														value="${fn:substring(reviewDto.reviewWriterString, 0, 3)}" />
													<%-- 이후 아이디를 * 처리 --%>
													<c:out value="***" />
												</c:when>
												<c:otherwise>
								...
								<td class="td-table-bottom"><a
									...
									class="table-title">${reviewDto.reviewContent}</a></td>
						...
						<c:forEach var="reviewDto" items="${list}">
							<tr class="tr-table-bottom">
								...
								<td class="td-table-bottom">
									...
									${reviewDto.reviewWriterString}</td>
								<td class="td-table-bottom"><a
									href="detail?reviewNo=${reviewDto.reviewNo}"
									class="table-title">${reviewDto.reviewContent}</a></td>
								<td class="td-table-bottom">${reviewDto.reviewScore}</td>
		...
		<div class="row center mt-30">
			<select name="column" class="field-column">
				...
				<option value="review_content"
					<c:if test="${param.column=='review_content'}">selected</c:if>>리뷰
					내용</option>
			</select> <input class="field" type="text" name="keyword">
